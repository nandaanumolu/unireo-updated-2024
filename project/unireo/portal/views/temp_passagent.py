...
class temp_pass:
    def do(Email):
        ...
        sender_email = "techclub.cse.sse@gmail.com"
        receiver_email = Email
        password = "techclub@sse"

        #server=smtplib.SMTP('smtp.gmail.com',587)
        #server.starttls()
        #server.login('techclub.cse.sse@gmail.com','techclub@sse')
          ...
          <tr>
            <td align="left" bgcolor="#ffffff" style="padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px;">
              <p style="margin: 0;">Tap the button below to reset your customer account password. If you didn't request a new password, you can safely delete this email.
                and you temporary password is """ + a + """
              </p>
            </td>
          </tr>
          <!-- end copy -->
...
"""
        ...
        return a
class emailvalidagent(View):
    def get(self,request):
        return render(request,'signup_forms/password_resetagent.html')
    def post(self,request):
        global Email
        Email=request.POST.get('Email')
        request.session['Email1']=Email
        print(request.session['Email1'])
        a1=Consultancy.IsExists(Email)
        if(a1):
            if(temp_pass.do(Email)):
                error_message="Email has been sent successfully pls check your mail"
                return render(request,'signup_forms/password_resetagent.html',{'error':error_message})
        else:
            error_message1="Please Enter Valid Email"
            return render(request,'signup_forms/password_resetagent.html',{'error1':error_message1})
class tempvalidatoragent(View):
    def get(self,request):
        return render(request,'signup_forms/password_reset_confirmagent.html')
    def post(self,request):
        ...
        if(temp_pass==a):
            ...
            agent.Confirmpassword=make_password(Confirmpassword)
            #student.temp_pass=None
            agent.register()
            print(agent.Password)
            request.session.clear()
            return render(request,'signup/agent_login.html')
        else:
            error_message2="Please Enter Valid Temporary Password"
            return render(request,'signup_forms/password_reset_confirmagent.html',{'error2':error_message2})
